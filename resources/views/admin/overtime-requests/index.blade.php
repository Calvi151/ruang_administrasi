    ...
    <div class="bg-white dark:bg-[#141C33] rounded-xl border border-outline-variant/40 dark:border-[#2A3654] shadow-sm overflow-visible animate-fade-in" style="animation-delay: 500ms;">
        ...
        <div class="flex flex-col divide-y divide-outline-variant/20 dark:divide-[#2A3654]">
            @forelse($overtimeRequests as $overtime)
                <div class="interactive-row grid grid-cols-12 gap-4 px-6 py-4 items-center bg-white dark:bg-[#141C33] rounded-lg m-1 animate-slide-in" style="animation-delay: {{ 600 + ($loop->index * 50) }}ms;">
                    ...
                    <div class="col-span-2 flex justify-end gap-2">
                        @if($overtime->status == 'pending')
                            ...
                            <button type="button" onclick="openModal('deleteModal-{{ $overtime->id }}')" class="px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-red-100 dark:bg-[#1D2847] dark:hover:bg-red-900/40 text-slate-500 hover:text-red-600 dark:text-[#8B93A8] dark:hover:text-red-400 font-semibold text-xs border border-outline-variant/30 dark:border-[#2A3654] transition-all flex items-center gap-1 active:scale-95 shadow-sm" title="Hapus">
                        ...
                        @else
                            ...
                            <div class="relative inline-block text-left">
                                ...
                                <div id="dropdown-{{ $overtime->id }}" class="action-dropdown hidden absolute right-0 mt-1 w-44 bg-white dark:bg-[#141C33] border border-outline-variant/40 dark:border-[#2A3654] rounded-xl shadow-lg z-[60] py-1 overflow-hidden">
                                    ...
                                    <button type="button" onclick="openModal('deleteModal-{{ $overtime->id }}')" class="w-full text-left px-4 py-2.5 text-xs font-semibold text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 flex items-center gap-2 transition-colors">
                ...
                {{-- Delete Modal --}}
                <div id="deleteModal-{{ $overtime->id }}" class="modal-overlay fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                    <div class="modal-content-box bg-white dark:bg-[#141C33] w-full max-w-md rounded-2xl shadow-2xl overflow-hidden border border-outline-variant/20 dark:border-[#2A3654]">
                        <div class="flex justify-between items-center p-5 border-b border-outline-variant/20 dark:border-[#2A3654] bg-red-50/50 dark:bg-red-900/10">
                            ...
                            <button type="button" onclick="closeModal('deleteModal-{{ $overtime->id }}')" class="text-outline hover:text-error transition-colors rounded-full p-1 hover:bg-surface-container-low dark:hover:bg-[#0F172E]">
                                <span class="material-symbols-outlined text-xl">close</span>
                            </button>
                        </div>
                        <form action="{{ route('overtime-requests.destroy', $overtime->id) }}" method="POST" class="p-6">
                            @csrf
                            @method('DELETE')
                            <div class="mb-6">
                                <p class="text-sm text-on-surface dark:text-[#E8E6E0]">Apakah Anda yakin ingin menghapus pengajuan lembur ini secara permanen? Data yang dihapus tidak dapat dikembalikan.</p>
                            </div>
                            <div class="flex justify-end gap-3 pt-2">
                                <button type="button" onclick="closeModal('deleteModal-{{ $overtime->id }}')" class="px-5 py-2.5 rounded-xl font-semibold text-on-surface-variant dark:text-[#8B93A8] hover:bg-slate-100 dark:hover:bg-[#0F172E] transition-colors">Batal</button>
