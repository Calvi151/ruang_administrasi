...
<div class="bg-surface-container dark:bg-ds-surface rounded-xl shadow-sm border border-outline-variant/50 dark:border-ds-border/50 overflow-hidden">
    ...
    <div class="px-6 py-4 border-b border-outline-variant/30 dark:border-ds-border/30 bg-surface-container-lowest dark:bg-ds-surface flex items-center justify-between">
        <div class="flex items-center gap-3">
            ...
            <div>
                <p class="font-label-sm text-label-sm text-on-surface-variant dark:text-ds-text-secondary uppercase tracking-wider">Nomor Surat</p>
                <h3 class="font-h3 text-h3 text-on-surface dark:text-ds-text-primary">{{ $outgoingLetter->letter_number }}</h3>
            </div>
        </div>
    ...
    <form action="{{ route('outgoing-letters.update', $outgoingLetter->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div id="edit-grid" class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6 min-w-0">
            ...
            <div class="space-y-5 min-w-0">
                ...
                <div>
                    <label class="block font-label-md text-label-md text-on-surface dark:text-ds-text-primary mb-1">Ganti Lampiran</label>
                    ...
                    <div class="mb-3 p-3 bg-primary-fixed/30 rounded-lg flex items-center justify-between border border-primary-fixed-dim/50">
                        ...
                        <a href="{{ asset('storage/' . $outgoingLetter->file_path) }}" target="_blank"
                            class="text-xs bg-primary dark:bg-ds-bg text-on-primary dark:text-ds-text-primary px-3 py-1.5 rounded-lg hover:opacity-90 transition-colors">Lihat</a>
                    </div>
                    @endif
                    <div class="flex justify-center px-6 pt-5 pb-6 border-2 border-outline-variant dark:border-ds-border border-dashed rounded-lg bg-surface-container-low dark:bg-ds-bg hover:bg-surface-container dark:bg-ds-surface dark:hover:bg-ds-hover transition-colors cursor-pointer group relative">
                        <div class="space-y-1 text-center">
                            <span class="material-symbols-outlined text-4xl text-on-surface-variant dark:text-ds-text-secondary group-hover:text-primary transition-colors">cloud_upload</span>
            ...
            <div class="space-y-5">
                ...
                <div class="tinymce-wrapper min-w-0 flex flex-col">
                    <label for="content" class="block font-label-md text-label-md text-on-surface dark:text-ds-text-primary mb-1">Isi Surat <span class="text-error">*</span></label>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Sinkronisasi kolom Perihal dengan editor
        ...
        if (subjectInput) {
            subjectInput.addEventListener('input', function() {
                ...
                if (editor) {
                    ...
                    if (placeholder) {
                        placeholder.textContent = this.value || '[PERIHAL]';
                    } else {
            ...
            // Trigger sekali saat halaman edit dimuat agar placeholder langsung terisi
            setTimeout(() => {
