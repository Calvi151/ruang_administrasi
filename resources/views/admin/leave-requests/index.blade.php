...
<div class="max-w-[container-max] mx-auto w-full">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        ...
        <div class="glass-card rounded-2xl p-5 border border-outline-variant/40 dark:border-[#2A3654] shadow-sm flex items-center justify-between group hover:border-amber-500 transition-colors cursor-default">
            <div>
        ...
        {{-- Card 2: Disetujui --}}
        <div class="glass-card rounded-2xl p-5 border border-outline-variant/40 dark:border-[#2A3654] shadow-sm flex items-center justify-between group hover:border-emerald-500 transition-colors cursor-default">
            <div>
        ...
        {{-- Card 3: Ditolak --}}
        <div class="glass-card rounded-2xl p-5 border border-outline-variant/40 dark:border-[#2A3654] shadow-sm flex items-center justify-between group hover:border-red-500 transition-colors cursor-default">
            <div>
        ...
        <div class="glass-card rounded-2xl p-5 border border-outline-variant/40 dark:border-[#2A3654] shadow-sm flex items-center justify-between group hover:border-blue-500 transition-colors cursor-default">
            <div>
    ...
    <div class="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-6">
        ...
        <div class="flex flex-wrap items-center gap-3">
            {{-- Filter by status --}}
            <div class="flex bg-white dark:bg-[#141C33] border border-outline-variant/50 dark:border-[#2A3654] rounded-lg overflow-hidden shadow-sm">
                <a href="{{ route('leave-requests.index') }}" class="px-3 py-2 text-xs font-semibold transition-colors {{ !request('status') ? 'bg-[#091426] dark:bg-ds-accent text-white dark:text-[#091426]' : 'text-on-surface-variant dark:text-[#8B93A8] hover:bg-slate-50 dark:hover:bg-[#1D2847]' }}">Semua</a>
                <a href="{{ route('leave-requests.index', ['status' => 'pending']) }}" class="px-3 py-2 text-xs font-semibold transition-colors border-l border-outline-variant/30 dark:border-[#2A3654] {{ request('status') == 'pending' ? 'bg-amber-500 text-white' : 'text-on-surface-variant dark:text-[#8B93A8] hover:bg-slate-50 dark:hover:bg-[#1D2847]' }}">
                    Pending
                    @if($stats['pending'] > 0)
                        <span class="ml-1 inline-flex items-center justify-center w-5 h-5 text-[10px] font-bold rounded-full {{ request('status') == 'pending' ? 'bg-white/30 text-white' : 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400' }}">{{ $stats['pending'] }}</span>
                    @endif
                </a>
                <a href="{{ route('leave-requests.index', ['status' => 'approved']) }}" class="px-3 py-2 text-xs font-semibold transition-colors border-l border-outline-variant/30 dark:border-[#2A3654] {{ request('status') == 'approved' ? 'bg-emerald-500 text-white' : 'text-on-surface-variant dark:text-[#8B93A8] hover:bg-slate-50 dark:hover:bg-[#1D2847]' }}">Disetujui</a>
                <a href="{{ route('leave-requests.index', ['status' => 'rejected']) }}" class="px-3 py-2 text-xs font-semibold transition-colors border-l border-outline-variant/30 dark:border-[#2A3654] {{ request('status') == 'rejected' ? 'bg-red-500 text-white' : 'text-on-surface-variant dark:text-[#8B93A8] hover:bg-slate-50 dark:hover:bg-[#1D2847]' }}">Ditolak</a>
            </div>

            {{-- Type filter --}}
            <select onchange="window.location.href='{{ route('leave-requests.index') }}?type='+this.value+'{{ request('status') ? '&status='.request('status') : '' }}'" class="bg-white dark:bg-[#141C33] border border-outline-variant/50 dark:border-[#2A3654] text-on-surface dark:text-[#E8E6E0] text-xs font-semibold px-3 py-2 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30 cursor-pointer">
            ...
            @forelse($leaveRequests as $leave)
                <div class="interactive-row grid grid-cols-12 gap-4 px-6 py-4 items-center bg-white dark:bg-[#141C33] rounded-lg m-1">
                    ...
                    <div class="col-span-2 flex justify-end gap-2">
                        @if($leave->status == 'pending')
                            {{-- ACC Button --}}
                            <form action="{{ route('leave-requests.approve', $leave->id) }}" method="POST" onsubmit="return confirm('Setujui pengajuan {{ $leave->type }} dari {{ $leave->employee->name }}?')">
                                @csrf
                                <button type="submit" class="px-3 py-1.5 rounded-lg bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-900/20 dark:hover:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 font-semibold text-xs border border-emerald-200 dark:border-emerald-700/50 transition-all flex items-center gap-1 active:scale-95 shadow-sm">
